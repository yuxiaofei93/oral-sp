...
患者对话关闭思考以降低延迟，发送给模型的上下文只包含本次问题命中的患者事实，不包含标准诊断、标准检查解释或评分答案。AI 辅助评分使用低强度思考与 JSON 输出，只发送评分标准、问诊消息、阶段提交和病例标准答案，不发送学生姓名或手机号。数据库保存最终评分、有效证据、置信度、评语、调用模型、耗时、token 数和错误码，不保存完整提示词或思维链。无效 JSON 会受控重试一次，越界分数或虚构证据会被拒绝。
