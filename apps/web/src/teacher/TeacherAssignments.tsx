        ...
        <form className="assignment-form" onSubmit={handleCreate}>
          ...
          <label>班级
            <select name="class_group_id" required>
              ...
              {availableClasses.map((item) => <option key={item.id} value={item.id}>{item.course_name} / {item.class_name}（{item.student_count} 人）</option>)}
            </select>
          </label>
        ...
        {assignments.map((assignment) => (
          <article key={assignment.id}>
            <div className="exam-list__header">
              <div><span>{assignment.course_name} / {assignment.class_name}</span><h3>{assignment.title}</h3><p>{assignment.case_title} · v{assignment.case_version_number} · {assignment.duration_minutes} 分钟</p></div>
