...
stateDiagram-v2
    ...
    Interview --> Interview: 提问、申请体格检查并编辑病例
    Interview --> Feedback: 最终交卷或超时
    Feedback --> [*]
```

### 4.2 单阶段与交卷锁定规则

- 问诊对话和病例编辑在最终交卷前始终同时开放。
- 病例草稿使用独立版本号自动保存，多页面冲突时拒绝静默覆盖。
- 学生申请检查后，记录申请项目、申请时间和查看时间，并把已释放的专科检查文字自动带入病例。
- 病例启用检查后主动询问时，患者按发布版本中的配置逐项提问；学生每回答一次再进入下一项，回答只进入对话记录。
- 最终交卷生成唯一、不可修改的结构化病例记录；交卷或超时后不再允许提问或编辑。
...
- 草稿保存后：仅显示未保存、保存中、已保存或保存失败状态，不提供答案提示。
...
病例编辑器采用纵向卡片表单，左侧提供固定导航，右侧完整展示病例内容；教师可上下滚动浏览和编辑，不要求普通教师直接编写大模型提示词。编辑内容自动保存，教师只需在完成后执行发布。
点击新建病例后，系统直接创建未命名草稿并进入基础信息。
...
模型遇到病例未定义的信息时不得编造，系统统一使用中性回答“这个我不太清楚。”。
...
- 是否必须由学生主动申请。
- 是否允许下载。
- 检查后患者主动询问采用系统默认、病例自定义或关闭，并配置有序问题列表和收尾语。
...
- 基本信息与专科检查只读；专科检查由已完成的体格检查自动带入。
- 主诉、现病史、既往史、家族史、诊断、处理和医嘱由学生编辑。
- 输入后自动保存草稿，但病例文字不直接计为“已向患者询问”。

### 6.3 最终交卷

- 学生点击“提交并完成问诊”后一次性提交当前病例草稿。
- 存在空白字段时列明缺项，允许返回补充或确认留空提交。
- 患者回答仍在生成时拒绝交卷；交卷成功后病例编辑器切换为只读。
...
- 语义路由和患者回答参考本次问诊全部学生与患者历史消息，不采用固定条数截断。
...
采用“确定性规则评分、教师最终复核”的模式。患者模型不评价学生，也不生成分数。
...
- 体格检查完成后能够按病例版本配置进行自然的一问一答，且不调用 AI 自动评分。

### 16.3 学生流程

- 学生可以在同一问诊过程中完成文字问诊和病例编辑，并最终提交一次。
- 口内照片和检查结果只能在满足释放条件后查看。
- 自动保存的病例草稿与最终病例记录由服务端保存。
- 网络刷新后能够恢复未结束会话。
